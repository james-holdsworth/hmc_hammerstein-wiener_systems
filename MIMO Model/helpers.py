    ...
    for j in range(0,ny):
        for i in range(0,nu):
            ...
            for s in sel:
                ...
                D_s = D_smps[s,[j],[i]]
                w, mag_samples[:, count], phase_samples[:, count] = signal.dbode((A_s,B_s,C_s,float(D_s),1), omega)
                count = count + 1
    ...
    for j in range(0,ny):
        for i in range(0,nu):
            ...
            for s in sel:
                ...
                D_s = D_smps[s,[j],[i]]
                w, mag_samples[:, count], phase_samples[:, count] = signal.dbode((A_s,B_s,C_s,float(D_s),1), omega)
                # phase_samples[:, count] = np.mod(phase_samples[:, count] + np.pi, 2*np.pi) - np.pi
                count = count + 1

            w, mag_true, phase_true = signal.dbode((A_t, B_t[:,[i]], C_t[[j],:], float(D_t[[j],[i]]),1), omega)
            # phase_true = np.mod(phase_true + np.pi, 2*np.pi) - np.pi
            # plot the samples
            plt.subplot(ny, nu, plt_idx)
            # h2, = plt.semilogx(w.flatten(), mag_samples[:, 0], color='green', alpha=0.1, label='samples')  # Bode magnitude plot
            # plt.semilogx(w.flatten(), mag_samples[:, 1:no_plot], color='green', alpha=0.1)  # Bode magnitude plot
